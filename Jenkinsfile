...
pipeline{
    ...
    stages {
        ...
        stage('Checkout from Git'){
            steps{
                git branch: 'main', url: 'https://github.com/disyakumah/mirosevice-dev-sce-ops.git'
            }
        }
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit ambok/reddit:latest "
                       sh "docker push ambok/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image ambok/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@18.218.160.216 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://18.191.159.144:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#dev-secops-cicd-amboki', //update and provide your channel name
